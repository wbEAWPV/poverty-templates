...
drop _min _max _median
gen logd5 = log(d5)
flagout logd5 [pw = hhweight], item(d0) z(2.5)
rename _flag flag3
drop _min _max _median
gen include = flag1 == 0 & flag2 == 0 & flag3 == 0
assert include == 0 if d3 < 0 // check obs with negative current value are excluded from construction of depreciation rates
